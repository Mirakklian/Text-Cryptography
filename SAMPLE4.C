#include<stdio.h>
#include<stdlib.h>
#include<math.h>
#include<conio.h>
#include<string.h>

        
    long int p,q,n,t,flag,e[100],d[100],temp[100],j,m[100],en[100],i;
     
    char msg[100];
     
    int prime(long int);
     
    void ce();
     
    long int cd(long int);
     
    void encrypt();
     
    void decrypt();
     
    int main() 
     {
	clrscr();
	printf("A PROGRAM TO DEMONESTRATE RSA ALGORITHM:- \n========================================");
    	printf("\nEnter first prime number:-  ");
     
    	scanf("%d",&p);
     
    	flag=prime(p);//check p is prime or not
     
    	if(flag==0)
		{
            printf("\nWrong input entered\n");
     
    		exit(1);
        }
     
    	printf("\nEnter another prime number:- ");
     
    	scanf("%d",&q);
     
    	flag=prime(q);
     
    	if(flag==0 || p==q) //check q is prime or not
		{
     
    		printf("\nWrong input entered");
     
    		exit(1);
     
    	}
     
    	printf("\nPlease enter the message:---\n");
     
    	fflush(stdin);
     
    	gets(msg); //get message from user to encript or decript
     
      	for (i=0;msg[i]!='\0';i++)
    	{
    		m[i]=msg[i];
		}  
		
		
     
    	n=p*q;  // where p and q are large prime
     
    	t=(p-1)*(q-1);
     
    	ce();

		     
	printf("\nThe possible values of 'e' And 'd' are:---------\n");
     
    	for (i=0;i<j-1;i++)
     
    	printf("\n%ld\t%ld",e[i],d[i]);
     
    	encrypt();
     
    	decrypt();
     
	getch();
	return 0;
     
    }
     
    int prime(long int pr) 
	{                          //check a number is prime or not
        int i;
     
    	j=sqrt(pr);
     
    	for (i=2;i<=j;i++) 
		{
            if(pr%i==0)
            {
    		    return 0;
            }
    	
		}
     
    	return 1;
     
    }
     
    void ce()   //a function to find the key for encryption
	{
        int k;
     
    	k=0;
     
    	for (i=2;i<t;i++)   //  t=(p-1)*(q-1)
		{
     
    		if(t%i==0)
    		{
			 continue;
     
            }
    		flag=prime(i); //check prime or not
     
    		if(flag==1 && i!=p && i!=q) 
			{
     
    			e[k]=i;
     
    			flag=cd(e[k]);
     
    			if(flag>0) 
				{
     
    				d[k]=flag;
     
    				k++;
     
    			}
     
    			if(k==99)
     
    			        break;
     
    		}
     
    	}
     
    }
     
    long int cd(long int x) 
	{
     
    	long int k=1;
     
    	while(1) 
		{
     
    		k=k+t;
     
    		if(k%x==0)
            {
		       return(k/x);
            }
    	}
     
    }
     
    void encrypt() 
	{
     
    	long int pt,ct,key=e[0],k,len;  //only e[0] is used as encryption key
     
    	i=0;
     
    	len=strlen(msg);
     
    	while(i!=len)
		 {
     
    		pt=m[i];
     
    		pt=pt-96;
     
    		k=1;
     
    		for (j=0;j<key;j++)
			 {
     
    			k=k*pt;
     
    			k=k%n;
     
    		}
     
    		temp[i]=k;
     
    		ct=k+96;
     
    		en[i]=ct;
     
    		i++;
     
    	}
     
    	en[i]=-1;
     
    	printf("\nThe Encrypted message:--------------\n");
     
    	for (i=0;en[i]!=-1;i++)
     
    	printf("%c",en[i]);
     
    }
     
    void decrypt() 
	{
     
    	long int pt,ct,key=d[0],k;   //only d[0] is used as decryption key
     
    	i=0;
     
    	while(en[i]!=-1) 
		{
     
    		ct=temp[i];
     
    		k=1;
     
    		for (j=0;j<key;j++)
		   {
     
    			k=k*ct;
     
    			k=k%n;
     
    	   }
     
    		pt=k+96;
     
    		m[i]=pt;
     
    		i++;
     
    	}
     
    	m[i]=-1;
     
    	printf("\nThe Decrypted message is:---\n");
     
    	for (i=0;m[i]!=-1;i++)
    	{
		
          printf("%c",m[i]);
        }
    }
     
